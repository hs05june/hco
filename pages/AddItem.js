...
const AddItem = (props) => {
    ...
    const [leftInStock,changeleftInStock] = useState()
    const [costPrice,changecostPrice] = useState()
    const [sellPrice,changesellPrice] = useState()
    const [alertAt,changealertAt] = useState()
  ...
  return (
    ...
    <form className='absolute m-auto z-50 md:top-[3vh] top-[15vh] md:left-[34vw] shadow-xl rounded-md bg-light_color'>
        ...
        <input onChange={(e)=>{setImage(e.target.files[0])}} type='file' name='image' id='image' className='hover:cursor-pointer' accept="image/x-png,image/gif,image/jpeg"/><br/>
